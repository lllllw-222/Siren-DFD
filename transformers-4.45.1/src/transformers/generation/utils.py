...
class GenerationMixin:
    ...
    def _get_logits_processor(
        ...
        if generation_config.do_sample:
            ...
            if generation_config.focus_decoding is False and generation_config.temperature is not None and generation_config.temperature != 1.0:
    ...
    def generate(
        ...
        **kwargs,
        ...
        if generation_mode == GenerationMode.ASSISTED_GENERATION:
            ...
            if generation_config.focus_decoding == True:
                result = self._focus_decoding_sample(
                    input_ids,
                    logits_processor=prepared_logits_processor,
                    stopping_criteria=prepared_stopping_criteria,
                    generation_config=generation_config,
    ...
    def _focus_decoding_sample(
        ...
        **model_kwargs,
        ...
        # prepare layers for focus decoding
        final_layer = self.config.get_text_config().num_hidden_layers
        ...
        if generation_config.focus_decoding_layers is not None:
            candidate_premature_layers = [i for i in generation_config.focus_decoding_layers if i < final_layer]
        else:
            candidate_premature_layers = range(start_layer, final_layer)

        lm_head = self.get_output_embeddings()
        if lm_head is None:
            raise ValueError("Focus decoding is not supported for models that don't have output embeddings.")
        ...
        while self._has_unfinished_sequences(
            ...
            dynamic_focus_weight = _get_focus_weight(
                candidate_premature_layers, candidate_premature_logits, final_logits, generation_config.focus_relative_top
            )
            
            if generation_config.focus_converter == None:
                # use exponential decay temperature converter as default
                dynamic_temperture = _exponential_decay_temperature_converter(
                    generation_config.base_temperature, dynamic_focus_weight, generation_config.focus_sigma
                )
            
            elif generation_config.focus_converter == "exponential_decay":
                dynamic_temperture = _exponential_decay_temperature_converter(
                    generation_config.base_temperature, dynamic_focus_weight, generation_config.focus_sigma
                )
            else:
                raise ValueError(f"Unsupported focus converter: {generation_config.focus_converter}")
                
            next_token_logits = final_logits / dynamic_temperture
...
def _get_focus_weight(
    candidate_premature_layers: List[int],
    candidate_premature_logits: Dict[int, torch.FloatTensor],
    final_logits: torch.FloatTensor,
    relative_top: float = 0.1,
    relative_top_value: float = -1e9,
) -> float:
    # (batch_size, layers, vocab_size)
    stacked_premature_logits = torch.stack([candidate_premature_logits[i] for i in candidate_premature_layers], dim=0).permute(1, 0, 2)
    
    # (batch_size, vocab_size)
    softmax_final_logits = F.softmax(final_logits, dim=-1)
    # (batch_size, vocab_size)
    relative_top_mask = softmax_final_logits > relative_top * torch.max(softmax_final_logits, dim=-1).values.unsqueeze(-1)

    # (batch_size, layers, vocab_size)
    masked_stacked_premature_logits = torch.where(relative_top_mask.unsqueeze(1), stacked_premature_logits, torch.full_like(stacked_premature_logits, relative_top_value))
    masked_log_softmax_premautre_logits = F.log_softmax(masked_stacked_premature_logits, dim=-1)
    
    # (batch_size, vocab_size)
    masked_final_logits = torch.where(relative_top_mask, final_logits, torch.full_like(final_logits, relative_top_value))
    masked_log_softmax_final_logits = F.log_softmax(masked_final_logits, dim=-1)
    
    pointwise_kl = F.kl_div(input=masked_log_softmax_premautre_logits, target=masked_log_softmax_final_logits.unsqueeze(1), log_target=True, reduction='none')
    
    # (batch_size)
    masked_pointwise_kl = pointwise_kl * relative_top_mask.unsqueeze(1)
    masked_kl = torch.sum(masked_pointwise_kl, dim=-1)

    return masked_kl.mean(dim=-1).item()
    
def _exponential_decay_temperature_converter(
    base_temperature: float = 1.0,
    dynamic_focus_weight: float = 1.0,
    focus_sigma: float = 0.25,
)-> float:
    
    if focus_sigma <= 0:
        raise ValueError("focus_sigma cannot be less than or equal to 0.")
    
    HALF = 0.5
    
    from math import pow
    return base_temperature * pow(HALF, dynamic_focus_weight / focus_sigma)